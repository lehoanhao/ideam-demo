...
watch(currentTag, () => fetchByTag())

function fetchByTag() {
  const channel = currentTag.value as CommunicationChannel | ''
  store.setFilters({ channel: channel || undefined })
  store.fetchRecords()
}
...
const pageTitle = computed(() => {
  if (currentTag.value === 'fax') return 'FAX送信履歴'
  if (currentTag.value === 'email') return 'メール送信履歴'
  return '送信履歴'
})
</script>
