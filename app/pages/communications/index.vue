...
const channelFilter = ref<CommunicationChannel | ''>('')
const searchQuery = ref('')

onMounted(() => store.fetchRecords())

watch(channelFilter, (val) => {
  store.setFilters({ channel: val || undefined })
  store.fetchRecords()
})
  ...
  <UDashboardPanel>
    ...
    <UDashboardToolbar>
      <template #left>
        ...
        <USelectMenu v-model="channelFilter" :items="channelOptions" value-key="value" class="w-36" placeholder="チャネル" />
      </template>
      <template #right>
