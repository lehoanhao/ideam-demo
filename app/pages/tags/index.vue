...
<template>
  <UDashboardPanel>
    <UDashboardNavbar title="タグ管理">
      <template #right>
        <UButton label="新規タグ" icon="i-lucide-plus" color="primary" @click="showAddModal = true" />
      </template>
    </UDashboardNavbar>

    <UDashboardToolbar>
      <template #left>
        <UInput v-model="searchQuery" icon="i-lucide-search" placeholder="タグ名で検索..." class="w-48" />
        <USelectMenu v-model="categoryFilter" :items="categoryOptions" value-key="value" class="w-40" placeholder="カテゴリ" />
      </template>
      <template #right>
    ...
    <UModal v-model:open="showAddModal">
      <template #content>
        <div class="p-6 space-y-4">
          <h3 class="text-lg font-semibold">新規タグ作成</h3>
          <div>
          ...
          <div class="flex justify-end gap-2">
            <UButton label="キャンセル" variant="ghost" color="neutral" @click="showAddModal = false" />
            <UButton label="作成" color="primary" @click="handleCreate" :disabled="!newTag.name.trim()" />
          </div>
        </div>
      </template>
    </UModal>
    ...
    <UModal v-model:open="showEditModal">
      <template #content>
        <div v-if="editingTag" class="p-6 space-y-4">
          <h3 class="text-lg font-semibold">タグ編集</h3>
          <div>
            <label class="block text-sm font-medium mb-1">タグ名</label>
          ...
          <div class="flex justify-end gap-2">
            <UButton label="キャンセル" variant="ghost" color="neutral" @click="showEditModal = false" />
            <UButton label="更新" color="primary" @click="handleUpdate" :disabled="!editingTag?.name?.trim()" />
          </div>
        </div>
      </template>
    </UModal>
